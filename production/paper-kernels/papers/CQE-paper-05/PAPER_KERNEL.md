# CQE-paper-05 - Oloid Path Carrier Kernel

Block: `block-00-papers-01-08`

Role: `paper_body_work`

This paper is treated as body work under the Paper 00 operating contract.

Suite edges: `CQE-paper-04` -> `CQE-paper-05` -> `CQE-paper-06`

Block edges: `CQE-paper-04` -> `CQE-paper-05` -> `CQE-paper-06`

Central thesis: Use curved/rolling carriers to show that transport need not be straight-line to preserve continuity.

## Claims

Status: `seeded`

Use curved/rolling carriers to show that transport need not be straight-line to preserve continuity.

Polished claim: a finite rolling-state carrier preserves path continuity by legal adjacent steps, emits head/tail dyads, and carries Paper 04 constraint payloads without requiring straight-line transport. It does not prove Rule 30 prediction.

Required next action: Connect this polished theorem to cqe_engine.formal and keep prediction claims as explicit open obligations.

## Math

Status: `source-bound`

Use the paper body axioms, lemmas, and formalism as the math intake.

Required next action: Promote every symbolic statement into a normalized claim row with assumptions and receipts.

## Formal, Normal, Closed-Form Algebra and Calculus

Status: `source-bound`

Use `production/formal-papers/CQE-paper-05/FORMAL_PAPER.md`, `01-CQE-formal/FORMAL.md`, and recovered algebra/PDF evidence.

Required next action: Split normal form, closed form, algebraic operators, calculus/window operators, and open obligations.

## By-Hand Reconstruction

Status: `source-bound`

Use 03-CQE-workbook/WORKBOOK.md plus the analog toolkit guides.

Required next action: Write the physical reconstruction steps and boundary-collision handling for the paper.

## Code Rebuild

Status: `source-bound`

Use 02-CQE-tool/TOOL.md and run.py when present.

Required next action: Bind a replayable command or mark the missing executable as an obligation.

## Installable Lib Bindings

Status: `seeded`

forgefactory.paper05_oloid_path_carrier

Required next action: Resolve this binding into production/lib-forge or mark it as an adapter route.

## Tests and Receipts

Status: `seeded`

Every validation and diagnostic must support hidden-guess ablation when training mode is enabled. Paper 05 now includes `production/formal-papers/CQE-paper-05/oloid_path_carrier_receipt.json`, which verifies rolling continuity, dyad validity, payload carrying, and rejection of discontinuous/invalid paths.

Required next action: Add positive, negative, boundary, and wrap tests with receipts.

## Deployment Kernel

Status: `seeded`

Deployable individually as CQE-paper-05 and selectable through the master suite.

Required next action: Expose paper, block, and suite selectors with no duplicate routing logic.

## Source Binding

- `body`: `production\papers\CQE-paper-05\PAPER-BODY.md` (present)
- `source`: `production\papers\CQE-paper-05\SOURCE.md` (present)
- `formal`: `production\papers\CQE-paper-05\01-CQE-formal\FORMAL.md` (present)
- `tool`: `production\papers\CQE-paper-05\02-CQE-tool\TOOL.md` (present)
- `tool_runner`: `production\papers\CQE-paper-05\02-CQE-tool\run.py` (present)
- `workbook`: `production\papers\CQE-paper-05\03-CQE-workbook\WORKBOOK.md` (present)
- `recovered_output`: `production\lib-forge\recovered\papers_output\CQE-paper-05.md` (present)
