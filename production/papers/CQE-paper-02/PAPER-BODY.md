...
Axiom 02.1 - Locality: every accepted claim must be readable through a local window before it is lifted to a larger frame.

Axiom 02.2 - Receipt Preservation: no transform is accepted unless its inputs, output, and unresolved residue can be logged.

Axiom 02.3 - Boundary Positivity: failed, partial, or mismatched routes are data. They become obligations or correction surfaces.

Axiom 02.4 - Analog Exposure Equivalence: if the claim is part of the main corpus, it can be exposed through a physical workbook analogue.

## Lemmas

Lemma 02.1 - If a local state preserves C and records L/R residue, it can be transported into a proof ledger without erasing unresolved alternatives.

Lemma 02.2 - If a tool output and supplemental workbook sheet encode the same center, boundary, and obligation state, they are equivalent receipts at different media layers.

Lemma 02.3 - A practical example is valid for this paper only when it demonstrates the same operation in a non-toy domain.
...
For Paper 02, the tool binding is:
...
**Domain:** inventory reconciliation: mismatched count becomes correction row, not garbage.
...
- Module: `forgefactory.paper02_correction_surface`
...
- [Rule30] Wolfram Rule 30 / elementary cellular automata. URL: https://mathworld.wolfram.com/Rule30.html Use: simple local rule producing complex global output.
- [ElementaryCA] Elementary cellular automata: 256 local binary rules. URL: https://mathworld.wolfram.com/ElementaryCellularAutomaton.html Use: finite binary rule-space framing.
...
- ForgeFactory registry receives or updates `forgefactory.paper02_correction_surface`.
