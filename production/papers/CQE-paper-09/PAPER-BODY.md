...
Axiom 09.1 - Locality: every accepted claim must be readable through a local window before it is lifted to a larger frame.

Axiom 09.2 - Receipt Preservation: no transform is accepted unless its inputs, output, and unresolved residue can be logged.

Axiom 09.3 - Boundary Positivity: failed, partial, or mismatched routes are data. They become obligations or correction surfaces.

Axiom 09.4 - Analog Exposure Equivalence: if the claim is part of the main corpus, it can be exposed through a physical workbook analogue.

## Lemmas

Lemma 09.1 - If a local state preserves C and records L/R residue, it can be transported into a proof ledger without erasing unresolved alternatives.

Lemma 09.2 - If a tool output and supplemental workbook sheet encode the same center, boundary, and obligation state, they are equivalent receipts at different media layers.

Lemma 09.3 - A practical example is valid for this paper only when it demonstrates the same operation in a non-toy domain.
...
For Paper 09, the tool binding is:
...
**Domain:** production schedule: local station timing becomes global throughput.
...
- Module: `forgefactory.paper09_hamiltonian_temporal`
...
- ForgeFactory registry receives or updates `forgefactory.paper09_hamiltonian_temporal`.
