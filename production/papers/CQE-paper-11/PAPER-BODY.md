...
Axiom 11.4 - Analog Exposure Equivalence: if the claim is part of the main corpus, it can be exposed through a physical workbook analogue.

## Lemmas

Lemma 11.1 - If a local state preserves C and records L/R residue, it can be transported into a proof ledger without erasing unresolved alternatives.

Lemma 11.2 - If a tool output and supplemental workbook sheet encode the same center, boundary, and obligation state, they are equivalent receipts at different media layers.

Lemma 11.3 - A practical example is valid for this paper only when it demonstrates the same operation in a non-toy domain.
...
For Paper 11, the tool binding is:
...
**Domain:** standards-compliance intake: nonconforming feature becomes exception trail.
...
- Module: `forgefactory.paper11_theory_admission`
...
- ForgeFactory registry receives or updates `forgefactory.paper11_theory_admission`.
