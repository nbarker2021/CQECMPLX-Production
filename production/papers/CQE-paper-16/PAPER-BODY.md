...
Axiom 16.4 - Analog Exposure Equivalence: if the claim is part of the main corpus, it can be exposed through a physical workbook analogue.

## Lemmas

Lemma 16.1 - If a local state preserves C and records L/R residue, it can be transported into a proof ledger without erasing unresolved alternatives.

Lemma 16.2 - If a tool output and supplemental workbook sheet encode the same center, boundary, and obligation state, they are equivalent receipts at different media layers.

Lemma 16.3 - A practical example is valid for this paper only when it demonstrates the same operation in a non-toy domain.
...
For Paper 16, the tool binding is:

```text
forgefactory.paper16_continuum_edges
```
...
**Domain:** measurement scale report: instrument precision defines unresolved remainder.
...
- Module: `forgefactory.paper16_continuum_edges`
...
- ForgeFactory registry receives or updates `forgefactory.paper16_continuum_edges`.
