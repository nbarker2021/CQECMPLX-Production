...
Axiom 22.1 - Locality: every accepted claim must be readable through a local window before it is lifted to a larger frame.

Axiom 22.2 - Receipt Preservation: no transform is accepted unless its inputs, output, and unresolved residue can be logged.

Axiom 22.3 - Boundary Positivity: failed, partial, or mismatched routes are data. They become obligations or correction surfaces.

Axiom 22.4 - Analog Exposure Equivalence: if the claim is part of the main corpus, it can be exposed through a physical workbook analogue.

## Lemmas

Lemma 22.1 - If a local state preserves C and records L/R residue, it can be transported into a proof ledger without erasing unresolved alternatives.

Lemma 22.2 - If a tool output and supplemental workbook sheet encode the same center, boundary, and obligation state, they are equivalent receipts at different media layers.

Lemma 22.3 - A practical example is valid for this paper only when it demonstrates the same operation in a non-toy domain.
...
For Paper 22, the tool binding is:

```text
forgefactory.metaforge
```
...
**Domain:** lightweight recycled-polymer honeycomb panel proposed and queued for simulation.
...
- [MechanicalMetamaterials] Bertoldi et al. Flexible mechanical metamaterials. URL: https://www.nature.com/articles/natrevmats201666 Use: architected materials and geometry-derived mechanical response.
- [AuxeticsReview] Greaves et al. Poisson's ratio and modern materials. URL: https://www.nature.com/articles/nmat3134 Use: auxetic/negative Poisson ratio material behavior.
...
- ForgeFactory registry receives or updates `forgefactory.metaforge`.
