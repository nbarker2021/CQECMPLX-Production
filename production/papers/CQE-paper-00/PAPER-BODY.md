...
Define the corpus-wide contract: every claim is a transported state with provenance, receipt, example, tool binding, and workbook representation.
...
Axiom 00.1 - Locality: every accepted claim must be readable through a local window before it is lifted to a larger frame.

Axiom 00.2 - Receipt Preservation: no transform is accepted unless its inputs, output, and unresolved residue can be logged.

Axiom 00.3 - Boundary Positivity: failed, partial, or mismatched routes are data. They become obligations or correction surfaces.

Axiom 00.4 - Analog Exposure Equivalence: if the claim is part of the main corpus, it can be exposed through a physical workbook analogue.

## Lemmas

Lemma 00.1 - If a local state preserves C and records L/R residue, it can be transported into a proof ledger without erasing unresolved alternatives.

Lemma 00.2 - If a tool output and supplemental workbook sheet encode the same center, boundary, and obligation state, they are equivalent receipts at different media layers.

Lemma 00.3 - A practical example is valid for this paper only when it demonstrates the same operation in a non-toy domain.
...
For Paper 00, the tool binding is:

```text
forgefactory.transport-contract
```
...
**Domain:** manufacturing quality trace: a part moves from raw material to inspected output with serial-number receipts.
...
- Module: `forgefactory.transport-contract`
...
- [JupyterRepro] Jupyter Project documentation. URL: https://jupyter.org/documentation Use: reproducible computational notebooks.
...
- ForgeFactory registry receives or updates `forgefactory.transport-contract`.
