...
Axiom 04.1 - Locality: every accepted claim must be readable through a local window before it is lifted to a larger frame.

Axiom 04.2 - Receipt Preservation: no transform is accepted unless its inputs, output, and unresolved residue can be logged.

Axiom 04.3 - Boundary Positivity: failed, partial, or mismatched routes are data. They become obligations or correction surfaces.

Axiom 04.4 - Analog Exposure Equivalence: if the claim is part of the main corpus, it can be exposed through a physical workbook analogue.

## Lemmas

Lemma 04.1 - If a local state preserves C and records L/R residue, it can be transported into a proof ledger without erasing unresolved alternatives.

Lemma 04.2 - If a tool output and supplemental workbook sheet encode the same center, boundary, and obligation state, they are equivalent receipts at different media layers.

Lemma 04.3 - A practical example is valid for this paper only when it demonstrates the same operation in a non-toy domain.
...
For Paper 04, the tool binding is:
...
**Domain:** civil-engineering crack map: defect becomes repair path and future load constraint.
...
- Module: `forgefactory.paper04_boundary_repair`
...
- ForgeFactory registry receives or updates `forgefactory.paper04_boundary_repair`.
