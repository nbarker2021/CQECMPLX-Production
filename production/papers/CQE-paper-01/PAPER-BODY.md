...
Axiom 01.1 - Locality: every accepted claim must be readable through a local window before it is lifted to a larger frame.

Axiom 01.2 - Receipt Preservation: no transform is accepted unless its inputs, output, and unresolved residue can be logged.

Axiom 01.3 - Boundary Positivity: failed, partial, or mismatched routes are data. They become obligations or correction surfaces.

Axiom 01.4 - Analog Exposure Equivalence: if the claim is part of the main corpus, it can be exposed through a physical workbook analogue.

## Lemmas

Lemma 01.1 - If a local state preserves C and records L/R residue, it can be transported into a proof ledger without erasing unresolved alternatives.

Lemma 01.2 - If a tool output and supplemental workbook sheet encode the same center, boundary, and obligation state, they are equivalent receipts at different media layers.

Lemma 01.3 - A practical example is valid for this paper only when it demonstrates the same operation in a non-toy domain.
...
For Paper 01, the tool binding is:
...
- Module: `forgefactory.paper01_lcr_chain_carrier`
...
- ForgeFactory registry receives or updates `forgefactory.paper01_lcr_chain_carrier`.
