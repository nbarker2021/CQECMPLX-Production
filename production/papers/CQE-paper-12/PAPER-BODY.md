...
Axiom 12.1 - Locality: every accepted claim must be readable through a local window before it is lifted to a larger frame.

Axiom 12.2 - Receipt Preservation: no transform is accepted unless its inputs, output, and unresolved residue can be logged.
...
For Paper 12, the tool binding is:
...
**Domain:** factory simulation with local station rules yielding global bottleneck patterns.
...
- Module: `forgefactory.paper12_ca_prediction`
...
- ForgeFactory registry receives or updates `forgefactory.paper12_ca_prediction`.
