...
def generate_paper(paper_id):
    ...
    if paper_id == "CQE-paper-32-obs":
        out.append("**Step**: 33 of 33 (Observation)")
    else:
        out.append(f"**Step**: {paper_id[-2:]} of 33")
    out.append("**Status**: Verified (bilateral)")
    out.append("")
    ...
    out.append(f"**Kit State**: {kit_data.get('total_tools', 0)} tools, {kit_data.get('colors', 0)} colors, {kit_data.get('digital_twins', 0)} digital twins")
    out.append(f"**New Tools Added**: {kit_data.get('new_tools', 0)}")
    out.append("")
    out.append("### Tools Active at This Step:")
    for t in kit_data.get("tools", []):
        out.append(f"- {t}")
    out.append("")
    ...
    out.append(f"- **Kit at step**: {kit_data.get('total_tools', 0)} tools, {kit_data.get('colors', 0)} colors, {kit_data.get('digital_twins', 0)} digital twins")
    out.append(f"- **New tools deployed**: {kit_data.get('new_tools', 0)}")
    out.append("- **Verification**: bilateral validator")
    out.append("")
