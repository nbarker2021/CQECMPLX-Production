...
class Kernel:
    ...
    def __init__(
        ...
        *,
        ...
        self.firmware_registry = FirmwareRegistry(firmware or DefaultFirmwareABI())
        self.split_bias = split_bias
        if self.split_bias not in (1, 2, 4, 8):
            raise ValueError("split_bias must be one of {1,2,4,8}")

        paths.ensure_dir(self.obligations_path.parent)
    ...
    def observe(
        ...
        *,
        ...
        frame = make_frame(req.raw_bytes, adapter="auto")
        ...
        bit_stream = tuple(int(b) for b in frame.sha256[:])[:len(req.raw_bytes) * 8] or tuple(
        ...
        # 7. Asymmetric admissibility gate on each gluon
        admission_results = [admit(g) for g in gluons]
        ...
        self._emit(
            ...
            {"gluon_count": len(gluons),
             ...
             "head_gluon": gluons[0].index if gluons else None,
             "tail_gluon": gluons[-1].index if gluons else None},
            input_hash=frame.sha256, output_hash=frame.sha256,
            request_id=req.request_id,
        )
    ...
    def firmware_manifest(self) -> Dict[str, Any]:
        ...
        base = self.firmware_registry.manifest()
        try:
            from ..firmware import lattice_forge_bridge
    ...
    def receipts_for_request(self, request_id: str) -> List[Receipt]:
        ...
        return [r for r in self.receipts.all()
                if r.payload.get("request_id") == request_id]
    ...
    def observe_packet(self, packet: Dict[str, Any]) -> ObservationResult:
        ...
        return self.observe(payload, mode=mode)
